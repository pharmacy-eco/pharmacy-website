...
const ButtonRoot: React.FC<IButtonRootProps> = ({
  ...
  const classColor = {
    primary: cn({
      "bg-blue-1d text-white hover:bg-blue-1d": variant === "solid",
      "bg-white text-blue-1d border border-blue-1d hover:bg-transparent": variant === "outline"
    }),
    success: cn({
  ...
  return (
    <Button
      ...
      className={cn(
        "relative inline-flex items-center justify-center gap-2 rounded-lg transition-all duration-300",
        classSize[size],
        classColor[color],
        className
      )}
    ...
    >
      ...
      <span className={cn({ "opacity-90": loading, "transition-opacity": true })}>{children}</span>
    </Button>
  );
};
