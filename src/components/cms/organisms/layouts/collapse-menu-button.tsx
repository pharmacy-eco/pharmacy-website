...
export function CollapseMenuButton({ icon, label, active, submenus, isOpen }: CollapseMenuButtonProps) {
  ...
  return isOpen ? (
    <Collapsible open={isCollapsed} onOpenChange={setIsCollapsed} className="w-full">
      <CollapsibleTrigger className="[&[data-state=open]>div>div>svg]:rotate-90 mb-1" asChild>
        <Button className="w-full justify-start h-10 shadow-none bg-transparent! hover:bg-transparent!">
          <div className="w-full items-center flex justify-between">
            ...
            <div className={cn("whitespace-nowrap", isOpen ? "translate-x-0 opacity-100" : "-translate-x-2 opacity-0")}>
              <ChevronRight size={16} className="transition-transform duration-300" color="#ffffff" />
            </div>
          </div>
        </Button>
      </CollapsibleTrigger>
      <CollapsibleContent className="overflow-hidden data-[state=closed]:animate-collapsible-up data-[state=open]:animate-collapsible-down">
        {submenus.map(({ href, label }, index) => (
          <Button
            key={index}
            className="w-full justify-start h-10 mb-1 shadow-none !bg-transparent hover:bg-transparent"
            asChild
          >
            <Link href={href}>
              <span className="mr-4">
                <DynamicIcon name="dot" width="20" height="20" color="#ffffff" />
              </span>
              <p
                className={cn(
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent side="right" sideOffset={25} align="start">
        ...
        <DropdownMenuSeparator />
        {submenus.map(({ href, label }, index) => (
          <DropdownMenuItem key={index} asChild className="hover:bg-transparent">
            <Link className="cursor-pointer" href={href}>
              <p className="max-w-[180px] truncate text-white">{label}</p>
            </Link>
          </DropdownMenuItem>
        ))}
